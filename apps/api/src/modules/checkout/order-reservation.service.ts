...
export class OrderReservationService implements OnModuleInit,OnModuleDestroy {
  ...
  async release(orderId:string){return this.prisma.$transaction(async tx=>{await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${orderId}))`;const order=await tx.order.findUnique({where:{id:orderId},include:{inventoryReservations:{where:{releasedAt:null}}}});if(!order||order.status!=='PENDING')return false;for(const reservation of order.inventoryReservations){await tx.$executeRaw`UPDATE "Inventory" SET "reservedQuantity" = GREATEST(0, "reservedQuantity" - ${reservation.quantity}) WHERE "variantId" = ${reservation.variantId}::uuid`;await tx.inventoryReservation.updateMany({where:{id:reservation.id,releasedAt:null},data:{releasedAt:new Date()}});}await tx.order.update({where:{id:order.id},data:{status:'CANCELLED'}});return true;},{isolationLevel:'Serializable'});}
